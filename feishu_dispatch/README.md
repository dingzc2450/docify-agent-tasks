...
建单描述末尾写一份溯源块（两个指纹 + 行号），与 metadata 双写。这不只是给人看的——**metadata 两层都 miss 时，会翻页拉全量 issue、按指纹搜一遍描述**。

理由：metadata 是建单之后**另一次 API 调用**写的，中间断掉就留下一张有单无键的孤儿单，对前两层完全隐形、下轮必被静默重建。而描述里的指纹串跟 issue 是**同一个 create 调用原子落地**的，不存在「建了单但键没写上」的窗口。命中后顺手把 metadata 补回去，下轮就回到第一层。

- 搜的是渲染出来的整行 `` `键`：`指纹` ``，不是裸 sha1 子串——MYD-272/273 这类讨论单正文里会引用指纹，裸搜会把讨论单当问题单命中，然后**静默漏建**一张真问题单。
- **必须翻页**：`--limit` 默认 50、服务端封顶 100，配 `--offset` 循环到 `has_more=false`。
- 全量索引每轮只拉一次并缓存，且**只服务第三层**；前两层照旧走 `--metadata k=v` 精确查，不要拿全量索引替代它们。
- v1 相撞的行在这一层同样禁用 v1，只认 v2。
...
跨轮次幂等靠**平台侧**的三层查重（metadata v2 → metadata v1 → 描述溯源块），三层都在 issue 上，不依赖本地文件。

- 建单默认不再回写 M=`处理中`，所以「下轮未解决筛选自动排除」那层保险没有了；`.daily_dispatch_state.json` 只在**单次运行内**兜底，autopilot 每次重新 checkout 本仓，它必为空。
- `set_issue_metadata` 是硬要求，不是尽力而为：写完**回读校验**（写入返回 0 不代表键真的落到单上）、失败**重试 3 次**（指数退避）；仍失败则记进 `.metadata_backfill_needed.json` 并**中止本轮**（平台没有 `issue delete`，删单回滚走不通）。
- ⚠️ **那个失败清单是会话级的**——文件在下次 checkout 时就没了，只当人在场时的提前报警用（启动读到非空则 `--execute` 拒跑，返回 6）。**别把它当跨轮次的闸门**：守着 metadata 的闸门不能比 metadata 本身更脆弱。真正兜住孤儿单的是上面第三层描述兜底。
- 查重调用本身失败则**直接停止**，宁可不建也不重复建。
- 历史教训：MYD-273 之前有 59 张历史单没有 metadata 键，对查重完全隐形（其中 36 张挂在 in_review——最可能被打回的状态），靠全量回填才补上。
